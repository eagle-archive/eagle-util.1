...
bool ParseRecord(string &line, VEHICLE_RECORD_NJ &r) {
    ...
    r.vechid = atol(row[0].c_str());
    r.lng = atof(row[1].c_str());
    r.lat = atof(row[2].c_str());
    r.speed = atoi(row[3].c_str());
    r.heading = atoi(row[4].c_str());
    r.gpstime = row[5];
    r.inload = atoi(row[6].c_str());
    r.inservice = atoi(row[7].c_str());
...
bool Test_NanjingAssign_UsingTileManager()
{
    ...
    cout << ElapsedTimeStr() << ": Enter Test_NanjingAssign_UsingTileManager()\n";
    std::ifstream in(infile);
    if (!in.good()) {
        cout << "Test_NanjingAssign_UsingTileManager: cannot open file " << infile << " for reading." << endl;
        return false;
    }
    remove(outfile);
    std::ofstream out(outfile);
    if (!out.good()) {
        cout << "Test_NanjingAssign_UsingTileManager: cannot open file " << infile << " for writing." << endl;
        return false;
    }
    ...
    while (GetLine(in, line)) {
        ...
        if (true == ParseRecord(line, record)) {
            ...
            out << record.vechid << ','
                << DoubleToStr(record.lng) << ',' << DoubleToStr(record.lat) << ','
                << record.speed << ','
                << DoubleToStr(record.speed) << ',' << record.heading << ','
                << '"' << record.gpstime << '"' << ','
                << record.inload << ',' << record.inservice << ','
                << assigned_seg_id1;
    ...
    cout << "Exit Test_NanjingAssign_UsingTileManager(), total count = " << count
